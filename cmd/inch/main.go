...
func (m *Main) ParseFlags(args []string) error {
	...
	tags := fs.String("t", "10", "Tag cardinality")
	fs.IntVar(&m.inch.PointsPerSeries, "p", 100, "Points per series")
	fs.StringVar(&m.inch.FieldPrefix, "field-prefix", "v", "Field key prefix")
	fs.IntVar(&m.inch.FieldsPerPoint, "f", 60, "Fields per point")
	fs.IntVar(&m.inch.BatchSize, "b", 5000, "Batch size")
	fs.StringVar(&m.inch.Database, "db", "vehicle", "Database to write to")
