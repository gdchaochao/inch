...
import (
	...
	"github.com/gdchaochao/inch/client/v2"
...
func (s *Simulator) generateBatches() <-chan []byte {
	...
	go func() {
		...
		//	pair := fmt.Sprintf("%s=%d%s", s.FieldPrefix, randNum, delim)
		//	if i > 0 {
		//		pair = fmt.Sprintf("%s%d=%d%s", s.FieldPrefix, i, randNum, delim)
		//	}
...
func generatorField(fieldsPerPoint int, fieldPrefix string) []byte {
	// Generate field string.
	var fields []byte
	for i := 0; i < fieldsPerPoint; i++ {
		var delim string
		if i < fieldsPerPoint - 1 {
			delim = ","
		}

		randId := rand.Intn(100)
		//fmt.Println("rand: %d %d \n", fieldId, randId)
		randNum := randomNumbers[i][randId]
		// First field doesn't have a number incremented.
		pair := fmt.Sprintf("%s=%d%s", fieldPrefix, randNum, delim)
		if i > 0 {
			pair = fmt.Sprintf("%s%d=%d%s", fieldPrefix, i, randNum, delim)
		}
		fields = append(fields, []byte(pair)...)
	}
	//fmt.Println(fields)
